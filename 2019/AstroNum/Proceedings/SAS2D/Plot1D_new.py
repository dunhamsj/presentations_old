...
c       = 2.99792458e8
alpha_N = np.sqrt( 1.0 + 2.0 * phi / c**2 )
psi_N   = ( 1.0 - 2.0 * phi / c**2 )**(1/4)
...
ax1.plot( x, alpha, 'k-', \
          markevery = me, markersize = ms, label = r'$\alpha$' )
ax1.plot( x, alpha_N, 'r-', \
          markevery = me, markersize = ms, label = r'$\alpha_{N}$' )
ax1.plot( x, h_av,  'k--', \
          markevery = me, markersize = ms, label = r'$h/c^2$'  )
ax1.plot( x, psi,   'k-.', \
          markevery = me, markersize = ms, label = r'$\psi$'   )
ax1.plot( x, psi_N, 'r-.', \
          markevery = me, markersize = ms, label = r'$\psi_{N}$' )
ax1.plot( x, W_av,  'k:', \
...
ax2 = fig.add_axes( [0.495,0.5,0.4,0.4], polar = True )
...
cbaxes = fig.add_axes( [0.36, 0.6, 0.03, 0.25] )
cbar = fig.colorbar( im, \
                     cax = cbaxes, \
                     ticks = [ np.min(Data) + 1.0e13, np.max(Data) ] )
cbar.ax.yaxis.set_ticks_position( 'right' )

cbar.ax.set_ylabel( \
  'Polytropic Constant\n$[\mathrm{erg/cm^{3}/(g/cm^{3})^{4/3}}]$', \
  rotation = 90, labelpad = -90.0 )
cbar.ax.set_yticklabels( \
  [ '{:.2e}'.format( np.min( Data ) + 1.0e13 ), \
    '{:.2e}'.format( np.max( Data ) ) ] )
#plt.show()
